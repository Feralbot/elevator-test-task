...
// [ ] 2. Не совсем логичная работа с данными из store в component/floor, часть данных берется из пропсов, но в тоже время подключено стор для вызова метода. Лучше выбрать один способо, либо общаться со стором напрямую, либо через пропсы и эвенты.
...
// [ ]- Четче разграничить ответственность между компонентами и store // убрать пропсы, брать данные из стора
